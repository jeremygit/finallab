...
OS_ECB* FTM_ChannelOutputEventSemaphores[FTM_CHANNELS_SIZE];
...
bool FTM_Init()
{
  // Init all Output Compare semaphores for the FTM Channels
  uint8_t channelSemCount;
  for (channelSemCount = 0; channelSemCount < FTM_CHANNELS_SIZE; channelSemCount++)
  {
    FTM_ChannelOutputEventSemaphores[channelSemCount] = OS_SemaphoreCreate(0);
  }
...
void __attribute__ ((interrupt)) FTM0_ISR(void)
{
  ...
  for (channelCount = 0; channelCount < FTM_CHANNELS_SIZE; channelCount++)
  {
	...
	if (CHANNEL_TIMER_FUNCTION(channelCount) == TIMER_FUNCTION_OUTPUT_COMPARE)
	{
	  ...
	  if (FTM_ChannelOutputEventSemaphores[channelCount])
	  {
	    OS_SemaphoreSignal(FTM_ChannelOutputEventSemaphores[channelCount]);
	  }
